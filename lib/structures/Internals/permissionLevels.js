...
// Member with Admin or Manage_Guild perms
	.add(6, ({ guild, member }) => guild && member && member.permissions.has('MANAGE_GUILD') || member.permissions.has('ADMINISTRATOR'), {
		fetch: true,
	}
	)
...
// Guild Owners
	.add(8, ({ guild, member }) =>
		guild && member === guild.owner, {
		fetch: true,
	}
	)

// Bot Owners permission for using lower commands
	.add(9, ({ author }) => require('../../../config').config.owners.includes(author.id), {
		break: true,
	}
	)
...
// Bot Owners
	.add(10, ({ author }) =>
		require('../../../config').config.owners.includes(author.id)
	);